...
describe('queue scope', () => {
  it('scopes Google Sheets queues by spreadsheet id and gid', () => {
    expect(
      getQueueScope('https://docs.google.com/spreadsheets/d/sheet_1/edit#gid=10&range=B2'),
    ).toBe('sheets:sheet_1:10');
    expect(getQueueScope('https://docs.google.com/spreadsheets/d/sheet_1/edit#gid=20')).toBe(
      'sheets:sheet_1:20',
    );
  });
  ...
  it('reads host and surface from the current url', () => {
    expect(getQueueHost('https://docs.google.com/forms/d/form_1/edit')).toBe('docs.google.com');
