...
describe('FrameworkSyncService preconditions', () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await Test.createTestingModule({
      providers: [FrameworkSyncService],
    }).compile();
  ...
  it('404s when framework instance not found (pre-lock)', async () => {
    ...
    await expect(
      service.sync({
        ...
        frameworkInstanceId: 'frm_missing',
  ...
  it('403s when instance belongs to a different org (pre-lock)', async () => {
    (db.frameworkInstance.findUnique as jest.Mock).mockResolvedValue({
      id: 'frm_1',
      organizationId: 'org_other',
      currentVersionId: 'fvr_1',
    });
    await expect(
      service.sync({
        organizationId: 'org_1',
  ...
  it('400s when target version belongs to a different framework (inside lock)', async () => {
    (db.frameworkInstance.findUnique as jest.Mock).mockResolvedValue({
      ...
      currentVersionId: 'fvr_current',
    });
    tx.frameworkInstance.findUnique.mockResolvedValue({
      id: 'frm_1',
      organizationId: 'org_1',
      frameworkId: 'frk_soc2',
      currentVersionId: 'fvr_current',
...
describe('FrameworkSyncService unpinned instances (FRAME-2)', () => {
  ...
  it('diffs from the earliest published version when the instance has no current version', async () => {
    ...
    tx.frameworkVersion.findUnique.mockResolvedValue({
      id: 'fvr_target',
      frameworkId: 'frk_soc2',
    });
    tx.frameworkVersion.findFirst.mockResolvedValue({
      id: 'fvr_earliest',
      frameworkId: 'frk_soc2',
    });
