...
describe('decideRunStatus', () => {
  ...
  describe('non-dynamic (never held)', () => {
    it('success → success', () => {
      expect(
        decideRunStatus({ resultStatus: 'success', isDynamic: false }),
      ).toBe('success');
    });
    it('failed → failed', () => {
      expect(
        decideRunStatus({ resultStatus: 'failed', isDynamic: false }),
      ).toBe('failed');
    });
    it('execution error → failed', () => {
      expect(decideRunStatus({ resultStatus: 'error', isDynamic: false })).toBe(
        'failed',
      );
    });
  });
  ...
  describe('dynamic (everything non-success → pending)', () => {
    it('success → success', () => {
      expect(
        decideRunStatus({ resultStatus: 'success', isDynamic: true }),
      ).toBe('success');
    });
    it('failed (a finding, a customer error, anything) → inconclusive (pending)', () => {
      expect(decideRunStatus({ resultStatus: 'failed', isDynamic: true })).toBe(
        'inconclusive',
      );
    });
    it('execution error → inconclusive (pending)', () => {
      expect(decideRunStatus({ resultStatus: 'error', isDynamic: true })).toBe(
        'inconclusive',
      );
    });
  });
