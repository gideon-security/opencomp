...
export async function runReconciliation(): Promise<ReconciliationResult> {
  ...
  for (const check of stuckChecks) {
    ...
    const result = await db.backgroundCheckRequest.updateMany({
      // Guard on the pointer read above: a retry that swapped in a fresh
      // vendor check between the select and this write must win. Dropping
      // the whole write (including the status) is correct — it was computed
