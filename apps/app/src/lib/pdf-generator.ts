import { AuditLog, Member, Organization, User } from '@db';
...
const measureTableFirstRowHeight = (tableNode: JSONContent, config: PDFConfig): number => {
  ...
  for (const cell of firstRow.content) {
    ...
    rowHeight = Math.max(rowHeight, lines.length * config.lineHeight + cellPadding * 2);
  }
  return rowHeight;
};
    ...
    switch (item.type) {
      case 'heading': {
        ...
        const headingLineCount = headingText
          ? config.doc.splitTextToSize(cleanTextForPDF(headingText), config.contentWidth).length
          : 0;
        ...
        const leadHeight = sectionLeadHeight(content, nodeIndex, config, spacingAfter);
...
const renderTable = (config: PDFConfig, tableNode: JSONContent) => {
  ...
  for (const row of rows) {
    ...
    const rowHeight = Math.max(
      minCellHeight,
      ...cellsInRow.map((c) => c.lines.length * config.lineHeight + cellPadding * 2),
    );
