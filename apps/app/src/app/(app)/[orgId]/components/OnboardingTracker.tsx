...
const ONBOARDING_STEPS = [
  { key: 'policies', labelKey: 'stepPolicies', order: 1 },
  { key: 'vendors', labelKey: 'stepVendors', order: 2 },
  { key: 'risk', labelKey: 'stepRisks', order: 3 },
  { key: 'linkage', labelKey: 'stepLinkage', order: 4 },
  { key: 'vendorMitigations', labelKey: 'stepVendorMitigations', order: 5 },
  { key: 'riskMitigations', labelKey: 'stepRiskMitigations', order: 6 },
] as const;

// The trigger job reports progress via English strings in its metadata
// (e.g. `stepStatus.currentStep`); match against those, independent of locale.
const STEP_METADATA_LABELS: Record<(typeof ONBOARDING_STEPS)[number]['key'], string> = {
  policies: 'Tailoring Policies',
  vendors: 'Creating Vendors',
  risk: 'Creating Risks',
  linkage: 'Linking to Controls',
  vendorMitigations: 'Assessing Vendors',
  riskMitigations: 'Assessing Risks',
};
...
export const OnboardingTracker = ({ onboarding }: { onboarding: Onboarding }) => {
  ...
  const currentStep = useMemo(() => {
    if (stepStatus.currentStep) {
      ...
      const step = ONBOARDING_STEPS.find((s) =>
        stepStatus.currentStep?.includes(STEP_METADATA_LABELS[s.key]),
      );
      return step || null;
    }
  ...
  const currentStepMessage = useMemo(() => {
    if (stepStatus.currentStep) {
      ...
      if (stepStatus.currentStep.includes(STEP_METADATA_LABELS.policies)) {
        if (stepStatus.policiesTotal > 0) {
          return t('tracker.tailoringPoliciesProgress', {
            completed: stepStatus.policiesCompleted,
            total: stepStatus.policiesTotal,
          });
        }
        return t('tracker.tailoringPoliciesEllipsis');
      }
      return stepStatus.currentStep;
    }
    if (currentStep) {
      return t(`tracker.${currentStep.labelKey}`);
    }
    return t('tracker.initializingEllipsis');
  }, [
  ...
  const uniqueVendorsInfo = useMemo(() => {
    ...
    const map = new Map<
      string,
      {
        ...
        rank: number;
        status: 'pending' | 'processing' | 'assessing' | 'completed';
      }
    >();
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-medium text-foreground">
                      {isCompleted ? t('tracker.setupComplete') : t('tracker.settingUpOrg')}
                    </p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {!isCompleted && (
                    <button
                      ...
                      aria-label={t('tracker.ariaExpand')}
                    >
                  ...
                  <button
                    ...
                    className="text-muted-foreground hover:text-foreground transition-colors"
                    aria-label={t('tracker.ariaClose')}
                  >
                    <X className="h-5 w-5" />
                  </button>
                </div>
    ...
    if (!run && !error) {
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-base font-medium text-foreground">
              {t('tracker.initializingEllipsis')}
            </p>
            <p className="text-muted-foreground text-sm mt-1">{t('tracker.checkingStatus')}</p>
          </div>
        </div>
      );
    }
    if (!run) {
      return (
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-warning text-base font-medium">{t('tracker.statusUnavailable')}</p>
            <p className="text-muted-foreground text-sm mt-1">
              {t('tracker.couldNotRetrieveStatus')}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="text-muted-foreground hover:text-foreground transition-colors"
              aria-label={t('tracker.ariaMinimize')}
            >
            ...
            <div className="flex flex-col gap-2.5 flex-1 overflow-y-auto min-h-0 pr-1">
              {ONBOARDING_STEPS.map((step) => {
                ...
                const isProcessing =
                  !isCompleted &&
                  ((step.key === 'policies' &&
                    Object.values(stepStatus.policiesStatus).some((s) => s === 'processing')) ||
                    (step.key === 'vendorMitigations' &&
                      Object.values(stepStatus.vendorsStatus).some(
                        (s) => s === 'processing' || s === 'assessing',
                      )) ||
                    (step.key === 'riskMitigations' &&
                      Object.values(stepStatus.risksStatus).some(
                        (s) => s === 'processing' || s === 'assessing',
                      )));
                    ...
                    <div key={step.key} className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        <div className="flex flex-1 items-center justify-between gap-2 min-w-0">
                          <span className={stepTextClass}>{t(`tracker.${step.labelKey}`)}</span>
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {uniqueVendorsInfo.map((vendor) => {
                              ...
                              const content = (
                                <>
                                  {done ? (
                                    ...
                                    <Loader2
                                      className="h-4 w-4 shrink-0 text-primary pointer-events-none"
                                      style={spinnerStyle}
                                    />
                                  ) : (
                                  ...
                                  <span
                                    className={`text-sm truncate pointer-events-none ${done || active ? 'text-primary' : 'text-muted-foreground'}`}
                                  >
                                    {vendor.name}
                                  </span>
                                </>
                              ...
                              return (
                                <div key={vendor.id} className="flex items-center gap-2">
                                  {done && orgId ? (
                                    <Link
                                      href={`/${orgId}/vendors/${vendor.id}?tab=treatment-plan`}
                            ...
                            {stepStatus.risksInfo.map((risk) => {
                              ...
                              return (
                                <div key={risk.id} className="flex items-center gap-2">
                                  {done && orgId ? (
                                    <Link
                                      href={`/${orgId}/risk/${risk.id}?tab=treatment-plan`}
                                      className="flex items-center gap-2 flex-1 min-w-0 hover:underline transition-all cursor-pointer"
                                    >
                                      {content}
                                    </Link>
                            ...
                            {stepStatus.policiesInfo.map((policy) => {
                              ...
                              const content = (
                                <>
                                  ...
                                  <span
                                    className={`text-sm truncate pointer-events-none ${done || processing ? 'text-primary' : 'text-muted-foreground'}`}
                                  >
                                    {policy.name}
                                  </span>
                                </>
                ...
                const total =
                  step.key === 'vendors'
                    ? uniqueVendorsCounts.total
                    : step.key === 'risk'
                      ? stepStatus.risksTotal
                      : null;
                const created =
                  step.key === 'vendors' && stepStatus.vendors
                    ? uniqueVendorsCounts.total
                    : step.key === 'risk' && stepStatus.risk
                      ? stepStatus.risksTotal
                      : 0;
                return (
                  <div key={step.key} className="flex items-center gap-2">
                    {stepIcon}
                    <span className={`${stepTextClass} flex-1`}>
          ...
          <div className="flex flex-col gap-4 h-full overflow-hidden">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <div className="flex flex-col gap-2">
                <p className="text-primary text-base font-medium">{t('tracker.orgReady')}</p>
                <p className="text-muted-foreground text-sm">{t('tracker.allStepsCompleted')}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2.5 shrink-0">
              {ONBOARDING_STEPS.map((step) => (
                <div key={step.key} className="flex items-center gap-2">
                  <CheckCircle2 className="text-primary h-5 w-5 shrink-0" />
                  <span className="text-sm text-primary">{t(`tracker.${step.labelKey}`)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-destructive text-base font-medium">
                  {t('tracker.setupNeedsAttention')}
                </p>
                <p className="text-muted-foreground text-sm mt-1">
                  {t('tracker.setupNeedsAttentionDescription')}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button size="sm" variant="outline" asChild>
                <a href="mailto:support@gideondefender.com">{t('tracker.contactSupport')}</a>
              </Button>
            </div>
          </div>
        );
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-warning text-base font-medium">
                {t('tracker.unknownStatusTitle')}
              </p>
              <p className="text-muted-foreground text-sm mt-1">
                {t('tracker.statusLabel', { status: exhaustiveCheck })}
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
