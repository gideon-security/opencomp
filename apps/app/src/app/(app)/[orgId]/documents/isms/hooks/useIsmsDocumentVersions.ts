 ...
 */
export function useIsmsDocumentVersions(documentId: string | null, organizationId?: string) {
  const { data, error, isLoading, mutate } = useSWR<IsmsVersionHistory>(
    documentId ? (['/v1/isms/documents', documentId, 'versions', organizationId] as const) : null,
    async ([base, id, , orgId]: readonly [string, string, string, string | undefined]) => {
      ...
      const response = await api.get<IsmsVersionHistory>(`${base}/${id}/versions`, orgId);
  ...
  const downloadVersion = async (versionId: string, format: IsmsExportFormat): Promise<void> => {
    ...
    try {
      ...
      toast.error(caught instanceof Error ? caught.message : 'Failed to download version');
    } finally {
      ...
      setDownloadingVersionId((current) => (current === versionId ? null : current));
    }
  };
