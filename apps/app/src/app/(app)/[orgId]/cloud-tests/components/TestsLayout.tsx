...
export function TestsLayout({ initialFindings, initialProviders, orgId }: TestsLayoutProps) {
  ...
  const handleRunScan = async (connectionId?: string): Promise<string | null> => {
    ...
    toast.message(
      t('cloudTests_startingSecurityScan', {
        name: targetProvider.displayName || targetProvider.name,
      }),
    );
    ...
    try {
      if (targetProvider.isLegacy) {
        ...
        if (!result.success) {
          ...
          toast.error(
            t('cloudTests_scanFailedWithErrors', { errors: result.errors?.join(', ') || '' }),
          );
          return null;
        }
      } else {
        ...
        if (response.error) {
          ...
          toast.error(
            t('cloudTests_scanProviderFailed', {
      ...
      toast.error(
        error instanceof Error
          ? `${t('cloudTests_scanCompleteFailed')} ${error.message}`
          : t('cloudTests_scanCompleteFailedRetry'),
      );
      return null;
    } finally {
      setIsScanning(false);
