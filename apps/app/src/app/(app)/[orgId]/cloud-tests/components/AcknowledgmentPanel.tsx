    ...
    return (
      <div className="space-y-3">
        <div className="flex items-start gap-2 rounded-md border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-950">
          ...
          <p className="text-sm text-yellow-700 dark:text-yellow-300">{acknowledgmentMessage}</p>
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => onAcknowledgmentChange(e.target.value || null)}
            className={acknowledged ? 'border-emerald-300 focus-visible:ring-emerald-500' : ''}
          />
        </div>
      </div>
    );
      ...
      <div className="space-y-3">
        ...
        <label className="flex cursor-pointer items-center gap-2">
          <input
            ...
            onChange={(e) => onAcknowledgmentChange(e.target.checked ? 'acknowledged' : null)}
            className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
          />
          <span className="text-sm text-muted-foreground">I understand the risks</span>
        </label>
      </div>
    );
  }
