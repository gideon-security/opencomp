 ...
 */
export function ResumeConnectStrip({ host, onResume, onDiscard }: ResumeConnectStripProps) {
  return (
    <div className="mb-4 rounded-lg border border-dashed border-border bg-muted/30 p-3">
      ...
      <div className="flex items-center gap-3 rounded-md border border-border bg-background px-3 py-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-[12.5px] text-foreground">Finish connecting to {host}</div>
          <div className="truncate text-[10.5px] text-muted-foreground">
            You started signing in but didn’t finish — resume it, or start a new one.
          </div>
        </div>
        <Button size="sm" variant="outline" onClick={onResume} iconLeft={<Renew size={12} />}>
          Resume
        </Button>
        <button
          type="button"
