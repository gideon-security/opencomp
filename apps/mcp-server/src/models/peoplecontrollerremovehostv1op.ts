/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from 'zod';
import { ClosedEnum } from '../types/enums.js';

/**
 * How the request was authenticated
 */
export const PeopleControllerRemoveHostV1AuthType = {
  ApiKey: 'api-key',
  Session: 'session',
} as const;
/**
 * How the request was authenticated
 */
export type PeopleControllerRemoveHostV1AuthType = ClosedEnum<
  typeof PeopleControllerRemoveHostV1AuthType
>;

export const PeopleControllerRemoveHostV1AuthType$zodSchema = z
  .enum(['api-key', 'session'])
  .describe('How the request was authenticated');

export type PeopleControllerRemoveHostV1Request = {
  id: string;
  hostId: number;
};

export const PeopleControllerRemoveHostV1Request$zodSchema: z.ZodType<PeopleControllerRemoveHostV1Request> =
  z.object({
    hostId: z.number().describe('FleetDM host ID'),
    id: z.string().describe('Member ID'),
  });

/**
 * Internal server error
 */
export type PeopleControllerRemoveHostV1InternalServerErrorResponseBody = {
  message?: string | undefined;
};

export const PeopleControllerRemoveHostV1InternalServerErrorResponseBody$zodSchema: z.ZodType<PeopleControllerRemoveHostV1InternalServerErrorResponseBody> =
  z
    .object({
      message: z.string().optional(),
    })
    .describe('Internal server error');

/**
 * Organization or member not found
 */
export type PeopleControllerRemoveHostV1NotFoundResponseBody = {
  message?: string | undefined;
};

export const PeopleControllerRemoveHostV1NotFoundResponseBody$zodSchema: z.ZodType<PeopleControllerRemoveHostV1NotFoundResponseBody> =
  z
    .object({
      message: z.string().optional(),
    })
    .describe('Organization or member not found');

/**
 * Unauthorized - Invalid authentication, insufficient permissions, or not organization owner
 */
export type PeopleControllerRemoveHostV1UnauthorizedResponseBody = {
  message?: string | undefined;
};

export const PeopleControllerRemoveHostV1UnauthorizedResponseBody$zodSchema: z.ZodType<PeopleControllerRemoveHostV1UnauthorizedResponseBody> =
  z
    .object({
      message: z.string().optional(),
    })
    .describe(
      'Unauthorized - Invalid authentication, insufficient permissions, or not organization owner',
    );

/**
 * Host removed from Fleet successfully
 */
export type PeopleControllerRemoveHostV1ResponseBody = {
  success?: boolean | undefined;
  authType?: PeopleControllerRemoveHostV1AuthType | undefined;
};

export const PeopleControllerRemoveHostV1ResponseBody$zodSchema: z.ZodType<PeopleControllerRemoveHostV1ResponseBody> =
  z
    .object({
      authType: PeopleControllerRemoveHostV1AuthType$zodSchema.optional().describe(
        'How the request was authenticated',
      ),
      success: z.boolean().optional().describe('Indicates successful removal'),
    })
    .describe('Host removed from Fleet successfully');

export type PeopleControllerRemoveHostV1Response =
  | PeopleControllerRemoveHostV1ResponseBody
  | PeopleControllerRemoveHostV1UnauthorizedResponseBody
  | PeopleControllerRemoveHostV1NotFoundResponseBody
  | PeopleControllerRemoveHostV1InternalServerErrorResponseBody;

export const PeopleControllerRemoveHostV1Response$zodSchema: z.ZodType<PeopleControllerRemoveHostV1Response> =
  z.union([
    z.lazy(() => PeopleControllerRemoveHostV1ResponseBody$zodSchema),
    z.lazy(() => PeopleControllerRemoveHostV1UnauthorizedResponseBody$zodSchema),
    z.lazy(() => PeopleControllerRemoveHostV1NotFoundResponseBody$zodSchema),
    z.lazy(() => PeopleControllerRemoveHostV1InternalServerErrorResponseBody$zodSchema),
  ]);
