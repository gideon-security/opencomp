/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from 'zod';

export type TriggerAutoAnswerResponseDto = {
  questionnaireId: string;
  runId: string;
  publicAccessToken: string;
  status: string;
  totalQuestions: number;
  answeredQuestions: number;
  message: string;
};

export const TriggerAutoAnswerResponseDto$zodSchema: z.ZodType<TriggerAutoAnswerResponseDto> =
  z.object({
    answeredQuestions: z
      .number()
      .describe(
        'Questions already answered at the moment generation was triggered (answers without a value are (re)generated).',
      ),
    message: z.string().describe('Human-readable summary of what to do next.'),
    publicAccessToken: z
      .string()
      .describe(
        'Token for tracking the run via Local trigger realtime. Optional — MCP/agent clients should poll GET /v1/questionnaire/:id instead.',
      ),
    questionnaireId: z.string().describe('The questionnaire whose answers are being generated.'),
    runId: z
      .string()
      .describe('Background run identifier for the generation job (Local trigger run id).'),
    status: z
      .string()
      .describe(
        "Always 'generating' on a successful trigger. Poll GET /v1/questionnaire/:id; answers are ready once answeredQuestions equals totalQuestions.",
      ),
    totalQuestions: z.number().describe('Total number of questions in the questionnaire.'),
  });
