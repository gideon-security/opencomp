...
const VALID_ANNOTATIONS = ['readOnly', 'destructive', 'idempotent', 'openWorld'] as const;
...
export function buildAnnotationFilter(annotations: string[] | undefined): MCPToolAnnotationFilter {
  ...
  for (const a of annotations) {
    if (!VALID_ANNOTATIONS.includes(a as AnnotationName)) {
      throw new Error(
        `Invalid annotation filter: "${a}". Valid values are: ${VALID_ANNOTATIONS.join(', ')}`,
      );
    }
  }
...
export type ToolDefinition<Args extends undefined | ZodRawShapeCompat = undefined> =
...
export async function formatResult(response: Response): Promise<CallToolResult> {
...
export function createRegisterTool(
  ...
  const toolMap = new Map<string, ToolDefinition<ZodRawShapeCompat | undefined>>();
  const registerTool = <A extends ZodRawShapeCompat | undefined>(tool: ToolDefinition<A>): void => {
    ...
    toolMap.set(tool.name, tool as ToolDefinition<ZodRawShapeCompat | undefined>);
...
export function registerDynamicTools(
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const tools: Array<{ name: string; description: string; scopes?: MCPScope[] | undefined }> =
        [];
  ...
  if (mcpScopes.length > 0) {
    const scopes = allowedScopes.size > 0 ? [...allowedScopes].sort() : [...mcpScopes];
...
export function buildSDK(
  ...
  const flags = {
    ...cliFlags,
    apikey: resolveHeader(headers, 'apikey', z.string(), cliFlags['apikey'], disableStaticAuth),
  };
