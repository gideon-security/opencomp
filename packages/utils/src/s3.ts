...
export function isValidS3Host(host: string): boolean {
  ...
  // Guard against ReDoS: limit length and avoid polynomial regex
  if (normalizedHost.length > 253) {
    return false;
  }

  // Must only contain allowed hostname characters
  if (!/^[a-z0-9.-]+$/.test(normalizedHost)) {
    return false;
  }

  // Strip the trailing '.amazonaws.com' and validate the prefix contains a known S3 marker
  const withoutSuffix = normalizedHost.slice(0, -'.amazonaws.com'.length);
  if (!withoutSuffix) {
    return false;
  }
  ...
  for (const label of labels) {
    ...
    if (label === 's3' || label === 's3-accesspoint' || label === 's3-control') {
      return true;
    }
