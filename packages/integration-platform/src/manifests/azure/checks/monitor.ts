...
async function runMonitorLoggingAlertingForSubscription(
  ctx: CheckContext,
  sub: string,
): Promise<void> {
  let evaluated = false;
  ...
  if (alerts !== null) {
    ...
    if (missing.length > 0) {
      ctx.fail({
        ...
        resourceId: sub,
        severity: 'medium',
        remediation: 'Create activity log alerts in Azure Monitor for these critical operations.',
  ...
  if (diag !== null) {
    ...
    if (hasExport) {
      ...
      ctx.fail({
        ...
        resourceId: sub,
        severity: 'medium',
        remediation: 'Configure a diagnostic setting to export subscription activity logs.',
