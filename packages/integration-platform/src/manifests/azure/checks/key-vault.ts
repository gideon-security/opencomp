...
async function listVaults(ctx: CheckContext, sub: string): Promise<KeyVault[] | null> {
...
async function runKeyVaultProtectionForSubscription(ctx: CheckContext, sub: string): Promise<void> {
  ...
  for (const v of vaults) {
    ...
    const isPublic =
      ...
      (p.publicNetworkAccess === 'Enabled' || p.networkAcls?.defaultAction === 'Allow');
    if (isPublic) {
...
export const keyVaultRbacCheck: IntegrationCheck = {
  ...
  description: 'Verify Key Vaults use Azure RBAC instead of legacy vault access policies.',
  service: 'key-vault',
