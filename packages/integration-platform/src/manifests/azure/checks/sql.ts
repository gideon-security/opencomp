...
async function listSqlServers(ctx: CheckContext, sub: string): Promise<SqlServer[] | null> {
...
async function runSqlAuditingForSubscription(ctx: CheckContext, sub: string): Promise<void> {
  ...
  for (const s of servers) {
    ...
    const auditing = await ctx
      .fetch<AuditingSetting>(`${ARM_BASE}${s.id}/auditingSettings/default?api-version=2021-11-01`)
      .catch((err) => {
