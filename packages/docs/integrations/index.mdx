...
- Background execution via Local trigger
...
- If all required variables are configured → Checks run automatically via Local trigger
- If variables needed → User configures them → Checks run
- Checks also run daily via scheduled Local trigger task
