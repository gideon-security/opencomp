...
- Local trigger dashboard for background jobs
...
if (errorMessage.includes('not enabled') || errorMessage.includes('not subscribed')) {
  ctx.pass({
    // Use pass() for informational, not fail()
...
const details = await Promise.all(repos.slice(0, 10).map((r) => ctx.fetch(`/repos/${r}`)));
```
...
run: async (ctx) => {
  ...
  const cacheValid = lastFetch && Date.now() - new Date(lastFetch).getTime() < 3600000; // 1 hour
...
const selectedRepos = await Promise.all(targetRepos.map((name) => ctx.fetch(`/repos/${name}`)));
```
...
Checks have a **15-minute timeout** (Local trigger default). For long-running checks:
...
export const secretScanningCheck: IntegrationCheck = {
  ...
  run: async (ctx) => {
    ...
    for (const repoName of targetRepos) {
      const alerts = await ctx.fetch<Alert[]>(`/repos/${repoName}/secret-scanning/alerts`);
