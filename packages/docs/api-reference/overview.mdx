...
description: 'Use the OpenComp API to automate SOC 2 evidence, policies, Trust Access, security questionnaires, tasks, and compliance workflows.'
---

The OpenComp API lets engineering, security, and compliance teams connect their internal systems to the compliance workflows they run in OpenComp.
...
> **Prefer to drive OpenComp from an AI assistant?** The same endpoints documented here are also available as tools in the [OpenComp MCP server](/mcp-server) — install it once in Claude Desktop, Cursor, ChatGPT, Codex, or any MCP-compatible client and ask in plain language.

## Authentication

Most organization automation uses the `X-API-Key` header. Create scoped API keys inside your OpenComp organization, keep them server-side, and grant only the permissions needed for the workflow you are building.

```bash
curl --request GET \
  --url "https://api.gideondefender.com/v1/organization" \
  --header "X-API-Key: $COMP_AI_API_KEY"
```

The reference covers OpenComp's product APIs across compliance operations, not just a single workflow. API-key backed endpoints are the integration contract for organization automation, while app session routes are kept out of the public reference.
...
The endpoint reference is generated from the NestJS OpenAPI specification used by the OpenComp API. Endpoint titles, descriptions, examples, visibility rules, and Mintlify metadata are maintained in the API source so generated pages stay aligned with product behavior as routes change.
...
Use production API calls against `https://api.gideondefender.com` and avoid storing API keys in client-side code.
