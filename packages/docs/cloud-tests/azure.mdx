...
<Note>
  OpenComp only ever issues `GET`/`list` API calls. It does not create, modify, or delete Azure resources, even when connected with a user that has write permissions.
</Note>
...
<Accordion title="Subscription not detected after sign-in" icon="magnifying-glass">
  OpenComp only sees subscriptions your signed-in account has access to. If you manage multiple subscriptions, sign in with an account that has **Reader** on the one you want to monitor, or use the service principal flow to target it explicitly.
</Accordion>
