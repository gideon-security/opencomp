---
title: "Cloud Tests"
description: "Continuously assess your cloud posture by connecting AWS, Azure, and GCP to OpenComp."
---

## What Cloud Tests do

Cloud Tests connect your cloud accounts to OpenComp and continuously evaluate them against security and compliance controls. Each finding is mapped to the frameworks you track (SOC 2, ISO 27001, CIS, PCI DSS, HIPAA), so cloud misconfigurations show up directly in your compliance posture.

All connections are **read-only by default**. OpenComp only makes `GET` / `list` calls against each cloud provider unless you explicitly opt in to auto-remediation (AWS).
