...
<Note>
Email delivery uses AWS SES (the email-worker process polls the SQS queue and calls SES `SendEmail`). The sender address must be a verified SES identity. AWS credentials are shared with S3 (`APP_AWS_*`).
</Note>
