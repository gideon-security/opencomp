...
<Card title="Download for Claude Desktop" icon="download" href="https://api.gideondefender.com/mcp/download/claude-desktop">
...
- Confirm the key hasn't been revoked in **OpenComp → Settings → API Keys**.
...
- The OpenComp API is briefly unavailable — try again in a moment or check status.
</Accordion>
...
No. The server runs locally on each user's machine via `npx`. OpenComp hosts nothing on your behalf, and you do not need to deploy or maintain any infrastructure.
</Accordion>

<Accordion title="Does the MCP server work offline?">
No. The server is a thin translator between your AI client and the OpenComp API at `https://api.gideondefender.com`. An internet connection is required.
</Accordion>

<Accordion title="Can multiple teammates use it at the same time?">
Yes. Each teammate installs the MCP server on their own machine with their own OpenComp API key. Their actions are independently authenticated and audit-logged.
</Accordion>

<Accordion title="What if my AI client doesn't support MCP yet?">
You can still use the [OpenComp API](/api-reference/overview) directly. The MCP server is an additional way to interact with the same endpoints — it doesn't replace anything.
</Accordion>

<Accordion title="How do new features show up?">
New tools and improvements ship with each OpenComp release. Because `npx -y` always pulls the latest published version, your AI client picks up new tools automatically the next time it starts the MCP server.
</Accordion>

<Accordion title="Where can I report an issue?">
Open an issue at [github.com/gideon-security/opencomp/issues](https://github.com/gideon-security/opencomp/issues) or email [hello@gideondefender.com](mailto:hello@gideondefender.com).
</Accordion>
