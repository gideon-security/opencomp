...
<Frame caption="Every run produces an audit-ready screenshot, stamped with the requirement, source, and timestamp.">
  <img src="/images/browser-evidence-example.png" alt="Automated browser evidence — a captured page with a OpenComp audit banner" />
</Frame>
...
<Note>
  Credentials are stored **encrypted in a secure vault**. OpenComp keeps only a reference to
...
<Accordion title="A connection says “Needs reconnect”" description="The saved session expired or a login step needs a human" icon="rotate">
  ...
  this every time a session expires, add the **authenticator setup key** so OpenComp can re-sign in
  on its own.
</Accordion>
...
<Accordion title="The steps for finding my authenticator key look generic" description="Vendor-specific vs. general guidance" icon="circle-question">
  For well-known vendors, OpenComp shows steps checked against their current help docs. For others it
