...
- Sign in on a schedule, **including 2FA** — OpenComp generates the one-time code for you.
...
1. **Connections** — the vendor logins OpenComp signs in with (**Settings → Browser connections**).
2. **Browser evidence** — the automations that use those logins, created inside an evidence task.
3. **Schedule** — OpenComp signs in, captures the page, and re-signs in on its own when a session expires.

<Note>
  You set up an automation inside a task, but the logins it uses are shared across your organization
  — connect a vendor once and any task can use it.
</Note>
...
3. You have the vendor login you want OpenComp to use — ideally a **dedicated service account** rather than a personal one.
...
2. OpenComp reads the sign-in page and shows the **exact fields it asks for** — it detects whether the login uses an email, a username, or extra fields like a workspace.
3. Choose how OpenComp signs in: with a **stored login**, or via **single sign-on (SSO)**, where you finish at your identity provider.
4. Enter the login. OpenComp signs in for you in a live browser. If a step needs you — a 2FA prompt, an email code — you can **take over in the same browser** and hand it back.

<Note>
  Credentials are stored **encrypted in a secure vault**. OpenComp keeps only a reference to them —
  never the raw password in plain text.
</Note>

## Unattended 2FA

This is what makes scheduled runs truly hands-off. If your login uses an authenticator app, give OpenComp the **setup key** once, and it generates the rotating 6-digit codes itself — so scheduled runs never wait on your phone.
...
<Frame caption="The setup-key field, with per-vendor steps — 'How do I find this key?' — generated from the vendor's current help docs.">
  <img
    src="/images/browser-2fa-setup-key.png"
    alt="Entering the authenticator setup key, with per-vendor guidance"
  />
</Frame>
...
- **Not sure where to find it?** Select **"How do I find this key?"** next to the field. OpenComp shows the steps for your specific vendor, checked against its current help docs.
...
<Tip>
  Use a **dedicated authenticator (MFA) device** for the automation — many vendors let you add more
  than one. It's revocable on its own and never touches your personal authenticator.
</Tip>

<Note>
  OpenComp never stores the rotating 6-digit code — only the setup key, encrypted — and generates a
...
1. **Describe what to capture** in plain English — for example, _"Open the organization's security settings and show that two-factor authentication is required for all members."_
2. _(Optional)_ Add a **pass/fail check** — for example, _"Two-factor authentication is required for everyone."_ Without one, the run captures a screenshot only.
...
<Frame caption="Testing an instruction — watch the AI drive a real browser live, then review the screenshot and verdict before saving.">
  <img
    src="/images/browser-test-run.png"
    alt="Live test run — the AI controlling a browser to capture evidence"
  />
</Frame>
...
- OpenComp signs in, navigates to the page you described, and captures a screenshot stamped with the **requirement, source URL, and capture time**. If you added a check, it records a **pass or fail** with a short reason.
- If the saved session has **expired**, OpenComp **re-signs in on its own** — using the stored login and a freshly generated 2FA code. You don't have to do anything.
...
<Accordion
  title="A connection says “Needs reconnect”"
  description="The saved session expired or a login step needs a human"
  icon="rotate"
>
  Click **Reconnect**, sign in again, and enter the 2FA code once in the live browser. To avoid this
  every time a session expires, add the **authenticator setup key** so OpenComp can re-sign in on
  its own.
</Accordion>

<Accordion
  title="The steps for finding my authenticator key look generic"
  description="Vendor-specific vs. general guidance"
  icon="circle-question"
>
  For well-known vendors, OpenComp shows steps checked against their current help docs. For others
  it shows the universal steps — the exact menu names may differ, so double-check in your account's
  security settings as you go.
</Accordion>

<Accordion
  title="The run paused for a captcha or “verify it's you”"
  description="Challenges that require a person"
  icon="shield"
>
...
<Accordion
  title="I don't see Browser connections in Settings"
...
1. Contact support at [support@gideondefender.com](mailto:support@gideondefender.com)
